...
describe('median', () => {
  it('should return 3', () => {
    let a = [1, 2, 3, 4, 5];
    expect(median(a, a.length, 0)).toStrictEqual(3);
  });
  ...
  it('array with some values not pushed', () => {
    let a = [1, 2, 3, 4];
    expect(median(a, a.length + 2, 0)).toStrictEqual(3);
  });
  it('median is the default value', () => {
    let a = [1, 2, 3];
    expect(median(a, a.length + 4, 0)).toStrictEqual(0);
  });
  it('compare with ml-array-median', () => {
    let a = [1, 2, 3];
    let b = [1, 2, 3, 5, 5, 5, 5];
    expect(median(a, a.length + 4, 5)).toStrictEqual(5);
    expect(mlMedian(b)).toStrictEqual(5);
  });
  it('check ml-array-median behavior', () => {
    let a = [1, 2, 3, 4, 5, 5];
    let commonResult = 3;
    expect(median(a, a.length, 0)).toStrictEqual(commonResult);
